    ...
    def longestConsecutive(self, nums: List[int]) -> int:
      ...
      for n in hash_set:
        ...
        if n - 1 not in hash_set:
            #Step 5: Every start sequence 
            start_consec = 1
            #Step 6: If we have our right neighbor in the hash_set, then we will increment this by one
            while n + start_consec in hash_set:
                start_consec += 1
            #Step 7: After going through the cosnecutive sequence, we will check if our max consecutive needs to be changed
